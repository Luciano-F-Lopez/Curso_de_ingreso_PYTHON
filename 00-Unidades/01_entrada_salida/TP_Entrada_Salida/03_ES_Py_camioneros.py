...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):
        ...
        toneladasNum = int(toneladas)

        toneladas_verdaderas = toneladasNum * 1000
        resultado_toneladas  = toneladas_verdaderas / 3500
        resultado_toneladas_Vera = (resultado_toneladas)

        alert("Camiones", resultado_toneladas_Vera)

    def btn_tiempo_llegada_on_click(self): 
        kilometros = self.txt_kilometros.get()

        kilometrosNum = float(kilometros)

        resultado = str(kilometrosNum / 90)

        alert("Destino De La Obra",f"El Camion Tardara en llegar {resultado} Horas")
